...
public interface SoforRepository extends JpaRepository<Sofor, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "delete from sofor p where p.tckn = :id", nativeQuery = true)
    ...
    @Transactional
    @Query(value = "select * from sofor s where s.id=:sofor_id", nativeQuery = true)
    Sofor findByIdForSofor(@Param("sofor_id") Long id);

    @Modifying
    @Transactional
    @Query(value = "update sofor s SET s.ad=:ad,s.soyad=:soyad,s.tckn=:tckn,s.tel=:tel where s.id=:sofor_id", nativeQuery = true)
