...
public class HatController {
    ...
    public ResponseEntity<HatResponse> insert(@RequestBody HatRequest hatRequest) throws Exception {
    ...
    public ResponseEntity<HatResponse> update(@RequestBody HatRequest hatRequest) throws Exception {
        return new ResponseEntity<>(hat.updateHat(hatRequest), HttpStatus.OK);
    }

    @GetMapping(path = "/findByNumberPlate/{number}")
    public ResponseEntity<List<Hat>> findByNumberPlate(@PathVariable Integer number) throws Exception {
